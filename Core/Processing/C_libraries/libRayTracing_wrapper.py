...
def WET_raytracing(SPR, beam_direction, ROI=[]):
  ...
  try:
    # import C library
    if(platform.system() == "Linux"): libRaytracing = ctypes.cdll.LoadLibrary("Process/C_libraries/libRayTracing.so")
    elif(platform.system() == "Windows"): libRaytracing = ctypes.cdll.LoadLibrary("Process/C_libraries/libRayTracing.dll")
    ...
    print('Warning: accelerated raytracing not enabled. The python implementation is used instead')
...
def compute_position_from_range(SPR, spot_positions, spot_directions, spot_ranges):
  ...
  try:
    ...
    float_array = np.ctypeslib.ndpointer(dtype=np.float32)
    int_array = np.ctypeslib.ndpointer(dtype=np.int32)
    libRaytracing.compute_position_from_range.argtypes = [float_array, float_array, float_array, int_array, float_array, float_array, float_array, ctypes.c_int]
    ...
    # call C function
    libRaytracing.compute_position_from_range(SPR.Image.astype(np.float32), Offset, PixelSpacing, GridSize, positions, directions, ranges, NumSpots)
...
def transport_spots_to_target(SPR, Target_mask, SpotGrid, direction):
  ...
  try:
    ...
    int_array = np.ctypeslib.ndpointer(dtype=np.int32)
    bool_array = np.ctypeslib.ndpointer(dtype=np.bool)
    libRaytracing.transport_spots_to_target.argtypes = [float_array, bool_array, float_array, float_array, int_array, float_array, float_array, float_array, ctypes.c_int]
    ...
    # call C function
    libRaytracing.transport_spots_to_target(SPR.Image.astype(np.float32), Target_mask.astype(np.bool), Offset, PixelSpacing, GridSize, positions, WETs, direction, NumSpots)
    ...
    for s in range(NumSpots):
      ...
      while True:      
        ...
        dist[1] = abs(((math.floor((SpotGrid["y"][s]-SPR.ImagePositionPatient[1])/SPR.PixelSpacing[1]) + float(direction[1]>0)) * SPR.PixelSpacing[1] + SPR.ImagePositionPatient[1] - SpotGrid["y"][s])/direction[1])
        dist[2] = abs(((math.floor((SpotGrid["z"][s]-SPR.ImagePositionPatient[2])/SPR.PixelSpacing[2]) + float(direction[2]>0)) * SPR.PixelSpacing[2] + SPR.ImagePositionPatient[2] - SpotGrid["z"][s])/direction[2])
        step = dist.min() + 1e-3
...
def transport_spots_inside_target(SPR, Target_mask, SpotGrid, direction, minWET, LayerSpacing):
  ...
  try:
    ...
    int_array = np.ctypeslib.ndpointer(dtype=np.int32)
    bool_array = np.ctypeslib.ndpointer(dtype=np.bool)
    libRaytracing.transport_spots_inside_target.argtypes = [float_array, bool_array, float_array, float_array, int_array, float_array, float_array, float_array, float_array, ctypes.c_int, ctypes.c_int, ctypes.c_float, ctypes.c_float]
    ...
    # call C function
    libRaytracing.transport_spots_inside_target(SPR.Image.astype(np.float32), Target_mask.astype(np.bool), Offset, PixelSpacing, GridSize, positions, WETs, Layers, direction, NumSpots, max_number_layers, minWET, LayerSpacing)
...
def transport_spots_inside_target_map(SPR, Target_mask, SpotGrid, direction, minWET, LayerSpacing):
  ...
  try:
    ...
    global layer_maps
    layer_maps.append(-1*np.ones(SPR.Image.shape))
